...
the SOURCE of the `lhtask` Claude Code plugin (two skills + bash templates that the bootstrap
...
1) CLAUDE.md — guidance for future Claude Code instances (the /init style). This is the agent-native
   ...
   LHTASK_FOREGROUND=1 as the sync lever, graceful no-op when claude/codegraph is missing, and the
   config defaults duplicated across lhtask.conf + lhtask_load_config + the inline defaults in
...
3) README.md — keep it EXTREMELY SHORT. Only: a 1–2 sentence description of what the plugin does, the
   ...
   files (the two skills, templates/lhtask.conf, templates/AGENTS.md, templates/githooks/README.md) each
